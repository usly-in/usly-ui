...
import { ImageIcon, BookOpen, Mail, Lock, Calendar, Trash2 } from "lucide-react";
...
const typeConfig = {
  moment: { icon: ImageIcon, label: "Moment", color: "text-blue-400", bg: "bg-blue-400/10" },
  chapter: { icon: BookOpen, label: "Chapter", color: "text-emerald-400", bg: "bg-emerald-400/10" },
  letter: { icon: Mail, label: "Letter", color: "text-[#e4a0a0]", bg: "bg-[#e4a0a0]/10" },
};
    ...
    >
      ...
      {isLocked && (
        <div className="h-32 bg-[#1c1c1c] flex flex-col items-center justify-center gap-2">
          ...
          <p className="text-xs text-[#888]">Opens {item.openAt ? format(new Date(item.openAt), "MMM d, yyyy") : "soon"}</p>
        </div>
      )}
      ...
      <div className="p-4">
        ...
        {item.caption && !isLocked && (
          <p className="text-xs text-[#888] line-clamp-2 mb-2">{item.caption}</p>
        )}
